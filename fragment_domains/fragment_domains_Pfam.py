...
fragments = pybedtools.BedTool('../primary_analysis/work-cached/pacbio-190731-facs-assign.bed')
fragments.head()
# Write a CSV version of the FRAGMENT dataset
fragments_df = fragments.to_dataframe().to_csv('output/fragment_chrom_coord.csv')
...
fragment_domains.to_csv('output/fragment_protein_coordinates.csv')
...
mlep = pd.read_csv('../primary_analysis/work-cached/joint-frag-mle-peak.csv')
mlep.head()
...
# Merging the frag_sorted (not collapsed) dataset with the mlep dataset
frags_sorted_mlep = pd.merge(frags_sorted.to_dataframe(), mlep, left_on=['chrom', 'start', 'end'], right_on=['chrom', 'start', 'end'], how='left')
frags_sorted_mlep.to_csv('output/frags_coordinates_mlep.csv')
frags_sorted_mlep = frags_sorted_mlep.drop('strand_y', axis=1).dropna(subset=['mlePeak'])
...
frags_sorted_mlep.to_csv('output/frags_coordinates_mlep.csv')
frag_domain_mlep.to_csv('output/fragment-protein-coord_90percsim.csv')
...
all_mleps = domain_tests.apply(get_mlep, axis=1)
all_domain_tests = domain_tests.copy()
all_domain_tests['mlePeak'] = all_mleps
all_domain_tests = all_domain_tests.sort_values('mlePeak', axis=0)
...
frags_coordinates.to_csv('output/fragment-protein-coordinates_updatedALL.csv')
...
frags_coord_mlep.to_csv('output/frags_coordinates_mlep.csv')
