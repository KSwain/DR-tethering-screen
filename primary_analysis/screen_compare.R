...
screens <- read.csv("~/revisions/frags_coordinates_mlep.csv", header = TRUE, sep=",")
...
png(file="~/revisions/libraryactscores_screencompare.png", width=1600,height=1600,res=250)
...
head(barcode_counts)
all_counts <- barcode_counts[,c(2,4,5,3,6,7,8,9,10,11)]
head(all_counts)

validated18 <- rbind(tif35, ebs1, ngr1, sro9, ded1, hsp26, jsn1, whi3, sbp1, yap1801, smy2, her1)
colnames(validated18) <- c("X", "barcode", "R", "FL", "L", "FR", "unsort", "frag", "yorf", "gene", "mlePeak18")
validated18
dim(validated18)
...
colnames(validated15) <- c("X", "barcode", "R", "FL", "L", "FR", "unsort", "frag", "yorf", "gene2", "mlePeak15")

dim(validated15)

validated18
validated15
validated<- cbind(validated15, validated18)
...
actscore
actscore$fragment <- c("Tif35(115-239)", "Ebs1(691-876)", "Ngr1(474-626)", "Sro9(14-151)", "Ded1(1-108)", "Hsp26(1-182)",
...
png(file="~/revisions/validated_screencompare.png", width=1800,height=1600,res=250)
